...
float Drive::get_left_position_in(){
  return( Encoder.position(deg) * drive_in_to_deg_ratio );
}
...
double curvet = 0.5; //Curvature
double satyt = 0.0001; //Max output
double Intyt = 0.0; //Min output
double deadZonet = 0.0; //Min input to get output > 0

double curve = 0.5; //Curvature
double saty = 0.00009; //Max output
double Inty = 0.0; //Min output
double deadZone = 0.0; //Min input to get output>0


void Drive::control_arcade(){
 double AxisT;

 double AxisoneCurve = abs(Controller.Axis1.value()) < deadZone ? 0 : saty * (signbit(Controller.Axis1.value()) * Inty + (1-Inty) * (curve * pow(Controller.Axis1.value(), 3) + (1-curve) * Controller.Axis1.value()));
 
if (Controller.Axis1.value() > 85 )
{
 AxisT = 0;
}
else 
 AxisT = abs(Controller.Axis3.value()) < deadZonet ? 0 : satyt * (signbit(Controller.Axis3.value()) * Intyt + (1-Intyt) * (curvet * pow(Controller.Axis3.value(), 3) + (1-curvet) * Controller.Axis3.value()));


 DriveL.spin(fwd, to_volt(AxisT + AxisoneCurve), volt);
 DriveR.spin(fwd, to_volt(AxisT - AxisoneCurve), volt);
}
