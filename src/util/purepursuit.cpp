...
void PurePursuit::executePath(vector<Point> Points){
    // Set the points and initialize necessary variables
    this->Points = Points;
    this->pointsLength = Points.size();
    this->lastFoundIndex = 0;

    // Loop to navigate through the path until the end is reached
    while (true){
        // Call chooseGoalPoint to get the next goal point
        Point goalPoint = chooseGoalPoint();
        auton::driveToPoint(goalPoint.x, goalPoint.y);
        // Implement your logic to move the robot towards the goalPoint
        // This might involve adjusting the orientation and setting motor controls

        // Update the current position and orientation of the robo

        // Break the loop if the goal point indicates reaching the end of the path
        if (goalPoint.lookAheadDistance == 0){
            // Optional: Add any final actions when the end of the path is reached
            break;
        }
    }
}
