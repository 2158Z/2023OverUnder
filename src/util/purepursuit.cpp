...
std::vector<Pose> getData(const asset& path) {
    ...
    for (std::string line : dataLines) {
        infoSink()->debug("read raw line {}", stringToHex(line));
        ...
        if (pointInput.size() != 3) {
            infoSink()->error("Failed to read path file! Are you using the right format? Raw line: {}",
                                      stringToHex(line));
            break;
        }
        Pose pathPoint(0, 0);
        ...
        infoSink()->debug("read point {}", pathPoint);
    }
 ...
 */
void follow(const asset& path, float lookahead, int timeout, bool forwards, bool async) {
    if (async) {
    ...
    std::vector<Pose> pathPoints = getData(path); // get list of path points
